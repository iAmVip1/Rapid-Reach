...
import { FaPhone, FaTrash, FaEye, FaCircle, FaSearch, FaFilter, FaFire, FaMapMarkerAlt, FaCalendar, FaCheck, FaCar, FaUser, FaEnvelope } from 'react-icons/fa';
import { Link } from 'react-router-dom';

export default function DashFireTruck() {
  const { currentUser } = useSelector((state) => state.user);
  const [fireTruckDrives, setFireTruckDrives] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState('all');
  const [onlineUsers, setOnlineUsers] = useState([]);

  // Fetch fire truck drives (include unapproved for admin review)
  useEffect(() => {
    const fetchFireTruckDrives = async () => {
      try {
        setLoading(true);
        const res = await fetch('/api/drive/admin/get?vehicleType=fire-truck&includeUnapproved=true', {
          credentials: 'include',
        });
        const data = await res.json();
        console.log('Fire Truck API Response:', data);
        
        if (res.ok) {
          const drives = data.data || [];
          setFireTruckDrives(drives);
          console.log('Fire truck drives found:', drives.length);
        } else {
          console.log('Failed to fetch fire truck drives:', data);
        }
      } catch (error) {
        console.log('Error fetching fire truck drives:', error.message);
      } finally {
        setLoading(false);
      }
    };

    fetchFireTruckDrives();
  }, []);

  // Mock online users for demo (replace with real socket implementation)
  useEffect(() => {
    const mockOnlineUsers = fireTruckDrives.slice(0, 2).map(drive => drive.userRef);
  ...
  const handleDeleteDrive = async (driveId) => {
    if (window.confirm('Are you sure you want to delete this fire truck drive? This action cannot be undone.')) {
      try {
        ...
        const data = await res.json();
        if (res.ok) {
          setFireTruckDrives(fireTruckDrives.filter(drive => drive._id !== driveId));
        } else {
          alert(data.message);
        }
      } catch (error) {
  ...
  const handleApprove = async (driveId) => {
    if (window.confirm('Approve this fire truck drive?')) {
      try {
        ...
        const data = await res.json();
        if (res.ok) {
          setFireTruckDrives(fireTruckDrives.map(drive => 
  ...
  const handleCallFireTruck = (drive) => {
  ...
  const filteredDrives = fireTruckDrives.filter(drive => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading fire truck drives...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Fire Truck Management</h1>
          <p className="text-gray-600">Manage all fire truck drives and their services</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Total Fire Trucks</p>
                <p className="text-2xl font-bold text-gray-900">{fireTruckDrives.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Online Trucks</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{new Set(fireTruckDrives.map(drive => drive.vechicleNumber)).size}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{fireTruckDrives.filter(drive => drive.userMail).length}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="relative">
              ...
              <select
                ...
                className="pl-10 pr-8 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200 appearance-none bg-white"
              >
                  ...
                  <tr key={drive._id} className="hover:bg-gray-50 transition-colors duration-200">
                      ...
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-12 w-12">
                          <img
                            ...
                            src={drive.userImage || drive.licenseUrls?.[0] || 'https://via.placeholder.com/48x48?text=F'}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          onClick={() => handleCallFireTruck(drive)}
                          className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-lg text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-all duration-200"
                        >
          ...
          {filteredDrives.map((drive) => (
            <div key={drive._id} className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => handleCallFireTruck(drive)}
                  className="flex-1 min-w-0 inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-all duration-200"
                >
        ...
        {filteredDrives.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">No fire truck drives found</h3>
